...
export async function GET(request) {
  try {
    ...
    if (search) {
      whereConditions.push('(name LIKE ? OR mobile LIKE ? OR room_number LIKE ?)')
      queryParams.push(`%${search}%`, `%${search}%`, `%${search}%`)
    }

    if (reason) {
      whereConditions.push('reason_for_leaving = ?')
    ...
    const [tenantHistory] = await pool.execute(`
      SELECT 
        *,
        DATEDIFF(rent_end, rent_start) as days_stayed,
        CASE 
          WHEN contract_completed = 1 THEN 'Completed'
    ...
    const [stats] = await pool.execute(`
      SELECT 
        ...
        SUM(CASE WHEN contract_completed = 1 THEN 1 ELSE 0 END) as completed_contracts,
        ...
        AVG(DATEDIFF(rent_end, rent_start)) as avg_stay_duration,
